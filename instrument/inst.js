...
// if( !process.argv[2] ) {
//   //console.log("Specify input file")
//   process.exit();
// }
// if( !process.argv[3] ) {
//   //console.log("Specify token message (usually extension path). Just use 'token' to generate token file.")
//   process.exit();
// }
//
// if( !process.argv[4] ) {
//     //console.log("Specify url to send the covered lines (coverage).")
//     process.exit();
// }
...
let testFiles = [
  ...
  ["ArgumentsCallee", 39],
  ["ajax",40],
  ["assignFunction",5], // TODO: assign variable in the if..else blockStatements
  ["assignIfInline",3], // TODO: assign variable in the if..else blockStatements
  ["assignIfInlineNested",2], // TODO: assign variable in the if..else blockStatements
  ["assigment",3],
  ["background",21],
  ["callexpr",9],
  ["conditional",8],
...
// var s = fs.readFileSync(process.argv[2], 'utf8');
  if (process.argv[2])
    ...
    testFiles.forEach(startAnalysis)
  ...
  function startAnalysis(item, index, arr) {
    ...
    function token() {
      // encoded = Buffer(process.argv[3]).toString('base64');
...
// Sanity check
    ...
    function parseIf(root, i) {
      if (root.body[i].consequent.type && root.body[i].consequent.type !== "BlockStatement") {
        ...
        let nodeToParse = root.body[i].consequent; //nuevo
        let newNode = {
          type: 'BlockStatement',
          body: [nodeToParse]
        };
      ...
      if (root.body[i].alternate && root.body[i].alternate.type && root.body[i].alternate.type !== "BlockStatement") {
        ...
        let nodeToParse = root.body[i].alternate; //nuevo
        let newNode = {
          type: 'BlockStatement',
          body: [nodeToParse]
        };
